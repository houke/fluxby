...
>(({ className, children, ...props }, ref) => {
  const closeLabel = translations[getStoredLanguage()].common.close;
  ...
  return (
    <DialogPortal>
      ...
      >
        {children}
        <DialogPrimitive.Close className='absolute top-4 right-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-1 focus:ring-ring focus:ring-offset-1 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground'>
          <X className='h-4 w-4' />
